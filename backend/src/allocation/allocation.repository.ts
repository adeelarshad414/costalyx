...
export interface AllocationRepository {
  listDimensions(query: PageQuery): Promise<Paginated<Dimension>>;
  ...
  listResourceTags(query: ListResourceTagsQueryDto): Promise<Paginated<ResourceTag>>;
  upsertResourceTag(
    input: UpsertResourceTagDto,
    actor: AuthenticatedUser,
    idempotencyKey: string
  ): Promise<ResourceTag>;
  summarizeDimensionMatches(dimensionId: string, resourceIds: string[]): Promise<DimensionMatchSummary>;
}

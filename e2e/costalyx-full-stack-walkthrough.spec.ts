...
import { openProductPage, signInAsAdmin } from './support/persona-helpers';
...
test.describe('Costalyx full-stack walkthrough', () => {
  ...
  test('renders every milestone surface with live auth and seeded data', async ({ page }) => {
    ...
    await openProductPage(page, 'Cloud portfolio');
    ...
    await openProductPage(page, 'Executive');
    await expect(page.getByRole('region', { name: 'Executive summary' })).toBeVisible({ timeout: 45000 });
    ...
    await openProductPage(page, 'Insights');
    await expect(page.getByRole('region', { name: 'Resource inventory and cost explorer' })).toBeVisible({ timeout: 45000 });
    ...
    await expect(page.getByRole('region', { name: 'Optimization recommendations' })).toBeVisible({ timeout: 45000 });

    await openProductPage(page, 'Billing Agent');
    await expect(page.getByRole('region', { name: 'Billing anomalies' })).toBeVisible({ timeout: 45000 });
    await expect(page.getByRole('region', { name: 'Billing anomalies' })).toContainText(/Stakeholder Statements|Agent Runs/);

    await openProductPage(page, 'Reporting');
    await expect(page.getByRole('region', { name: 'Reporting and saved views' })).toBeVisible({ timeout: 45000 });
    ...
    await expect(page.getByRole('region', { name: 'Allocation and dynamic tagging' })).toBeVisible({ timeout: 45000 });

    await openProductPage(page, 'Governance');
    await expect(page.getByRole('region', { name: 'Access and trust controls' })).toBeVisible({ timeout: 45000 });

...
_Last updated: 2026-07-04 02:08:20 PKT_
...
- [ ] Live browser Keycloak E2E completed (blocked by unavailable live Keycloak stack; `npm run test:e2e` reported 1 skipped login test)
...
- **Blocker:** Milestone F live browser proof shares the same local Keycloak
  blocker as Milestones A, B, C, D, and E.
  **Impact:** Backend/contract/frontend executive and TCO evidence are
  verified, but the `07-FRONTEND-BACKEND-WIRING.md` live-login requirement
  and `08-TESTING-STRATEGY.md` E2E persona requirement are still not
  satisfied for the executive surfaces milestone.
  **What was verified instead:** Direct API integration tests verify
  viewer-authenticated Executive Summary, PDF export, and TCO estimate
  behavior after ingesting the AWS fixture; service tests verify spend as %
  of revenue/budget baseline, top mover selection, and TCO pricing via the
  same decimal multiplication helper used by ingestion; frontend tests
  verify KPIs, top movers, PDF export, TCO comparison, empty states, and
  error retry; live backend contract verifies the executive summary and TCO
  fixture reconciliation against a real Nest HTTP server; and the opt-in
  Postgres bundle verifies the underlying persisted cost-model path still
  passes against local Docker Postgres.

- **Blocker:** Milestone G live browser proof shares the same local Keycloak
  blocker as Milestones A, B, C, D, E, and F.
  **Impact:** Backend/contract/frontend reporting evidence and PostgreSQL
  saved-view persistence are verified, but the
  `07-FRONTEND-BACKEND-WIRING.md` live-login requirement and
  `08-TESTING-STRATEGY.md` E2E persona requirement are still not satisfied
  for the reporting/governance milestone.
  **What was verified instead:** Direct API integration tests verify canned
  report catalog availability, Admin-created Viewer-shared views, Viewer
  view listing, cost summary scoping, attempted provider override
  prevention, and report-run scoping; service tests verify all required
  canned report categories and provider-scoped report execution; frontend
  tests verify report gallery, active shared views, scoped run result,
  Admin create-view action, empty state, and error retry; live backend
  contract verifies saved view scoping against a real Nest HTTP server; and
  opt-in real Postgres tests verify view persistence and active-view summary
  scoping across app instances.
...
- (none yet — Milestones A, B, C, D, E, F, and G are intentionally marked `Blocked`, not
